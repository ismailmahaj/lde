...
if (!have_posts()) : 
	echo '<div class="cmsmasters_search_zero">' . 
		'<h4>' . esc_html__('Sorry, but nothing matched your search terms. Please try again with some different keywords.', 'dance-studio') . '</h4>' . 
		'<br />';
	...
	while (have_posts()) : the_post(); 
		...
		$current_tax .= (has_term('pl-categs') ? 'pl-categs' : '');
		...
		<article id="post-<?php the_ID(); ?>" <?php post_class('cmsmasters_archive_type'); ?>>
			...
			if (!post_password_required() && has_post_thumbnail()) {
				echo '<div class="cmsmasters_archive_item_img_wrap">';
					dance_studio_thumb(get_the_ID(), 'cmsmasters-square-thumb', false, 'img_' . get_the_ID(), true, true, true, true, false);
				echo '</div>';
			}
			?>
				...
				if ($current_tax != '' || $current_tax == '') {
					echo '<footer class="cmsmasters_archive_item_info entry-meta">';
						...
						'<span class="cmsmasters_archive_item_user_name">' . 
							'<a href="' . esc_url(get_author_posts_url(get_the_author_meta('ID'))) . '" rel="author" title="' . esc_attr__('Posts by', 'dance-studio') . ' ' . get_the_author_meta('display_name') . '">' . get_the_author_meta('display_name') . '</a>' . 
						'</span>';
						
						
						if ($current_tax != '') {
							echo '<span class="cmsmasters_archive_item_category">' . 
								dance_studio_get_the_category_list(get_the_ID(), $current_tax, ', ') .
							'</span>';
						};
						
						echo 
						'<span class="cmsmasters_archive_item_date_wrap fr">' . 
							esc_html__('On', 'dance-studio') . ' ' . 
